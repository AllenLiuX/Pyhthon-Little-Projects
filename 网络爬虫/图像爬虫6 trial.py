...
url = 'http://image.baidu.com/search/index?tn=baiduimage&ps=1&ct=201326592&lm=-1&cl=2&nc=1&ie=utf-8&word=%E5%BC%A0%E6%9F%8F%E8%8A%9D'
...
# ^abc.*?qwe$
pic_url = soup.find_all('img', src=re.compile(r'^data:image/j.peg/base64.*?'))
...
for url in pic_url:
    ...
    try:
        ...
        print('当前图片无法下载')
        continue
    file_name = "image3/" + str(i) + ".jpg"  # 拼接图片名
    print(file_name)
    # 将图片存入本地
